...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  return {
    title: `${category.name} - KataSEO Blog`,
    description: `Tất cả bài viết về ${category.name}. Khám phá những kiến thức chuyên sâu và tips hữu ích.`,
    keywords: `${category.name}, SEO, Digital Marketing, Blog`,
...
export default async function CategoryPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    '@type': 'CollectionPage',
    name: `${category.name} - KataSEO Blog`,
    description: `Tất cả bài viết về ${category.name}`,
    url: `${process.env.NEXT_PUBLIC_SITE_URL}/category/${category.slug}`,
    mainEntity: {
      '@type': 'ItemList',
      numberOfItems: category._count.posts,
        ...
        <section className="bg-white border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <p className="text-lg text-gray-700 max-w-3xl">
              Khám phá tất cả bài viết về {category.name}. Những kiến thức chuyên sâu và 
              tips hữu ích để giúp bạn thành công trong lĩnh vực này.
            </p>
          </div>
        </section>
          ...
          {category.posts.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                Chưa có bài viết nào trong {category.name}
              </h3>
                  ...
                  <article key={post.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow">
                    ...
                    <div className="p-6">
                      <div className="flex flex-wrap gap-2 mb-3">
                        {post.categories.filter(cat => cat.id !== category.id).slice(0, 2).map(cat => (
                          <Link
                            key={cat.id}
